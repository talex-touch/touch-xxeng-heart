...
export async function requestReplacementCandidates(
  ...
  const data = await postAiJson<AiReplacementResponse>('replacement', {
    ...
    instruction: [
      ...
      '1) General everyday Chinese words and short phrases — this is the primary goal, NOT limited to technical vocabulary: set original to the Chinese expression and replacement to the natural English expression a native speaker would use. Preserve the exact grammatical role in the supplied text. When English form depends on context, original must be the shortest context-bearing Chinese phrase with at least four CJK characters and replacement must replace that whole phrase in place; for example, use "验证您的身份" -> "validate your identity", never bare "验证" -> "validation" where the source needs a verb. Do NOT extract single Chinese characters or ambiguous one-character terms.',
